...
const AccommodationOverview = () => {
  ...
  const fetchAccommodationCounts = async () => {
    try {
      ...
      setActiveCount(response.data.inactive || 0); // Now set inactive count to active
      setInactiveCount(response.data.active || 0); // Now set active count to inactive
    } catch (error) {
      ...
      <div style={styles.overview}>
        ...
        <div style={styles.summarySection}>
          <div style={{ ...styles.summaryCard, backgroundColor: '#4caf50' }}>
            ...
            <p>{inactiveCount}</p> {/* Display inactive count as active */}
          </div>
          <div style={{ ...styles.summaryCard, backgroundColor: '#f44336' }}>
            <h3>Inactive Accommodations</h3>
            <p>{activeCount}</p> {/* Display active count as inactive */}
          </div>
        </div>
        ...
        <table style={styles.accommodationTable}>
          <thead>
            <tr>
              ...
              <th style={styles.accommodationTableHeader}>Inactive Accommodations</th> {/* Inactive -> Active */}
              <th style={styles.accommodationTableHeader}>Active Accommodations</th> {/* Active -> Inactive */}
            </tr>
          </thead>
          <tbody>
            {accommodations.map((hotel, index) => (
              ...
              >
                ...
                <td style={styles.accommodationTableCell}>{hotel.active_rooms}</td> {/* Display active rooms in inactive column */}
                <td style={styles.accommodationTableCell}>{hotel.inactive_rooms}</td> {/* Display inactive rooms in active column */}
              </tr>
            ))}
          </tbody>
        </table>
